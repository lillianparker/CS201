...
public class BinarySearchAutocomplete implements Autocompletor {
	...
	public List<Term> topMatches(String prefix, int k) {
		...
		PriorityQueue<Term> q = new PriorityQueue<>(Comparator.comparing(Term::getWeight));
		for (int j = first; j <= last; j++) {
			if (q.size() < k) {
				q.add(myTerms[j]);
			}
			else if (q.peek().getWeight() < myTerms[j].getWeight()) {
				q.remove();
				q.add(myTerms[j]);
			}
		}
		int results = Math.min(q.size(), k);
		LinkedList<Term> fin = new LinkedList<>();
		for (int x = 0; x <results; x++) {
			fin.addFirst(q.remove());
		}
		return fin;

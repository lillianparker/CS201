...
public class    PrefixComparator implements Comparator<Term> {
    ...
    public int compare(Term v, Term w) {
            ...
            int length = Math.min(vStr.length(), wStr.length());
            int its = Math.min(length, myPrefixSize);
            for(int i = 0; i < its; i++){
                if(vStr.charAt(i) > wStr.charAt(i))   
                    return 1;
                else if(vStr.charAt(i) < wStr.charAt(i))   
                    return -1;
            }
            if(myPrefixSize > length)
                if(vStr.length() < wStr.length()){
                    return -1;
                }
                else if(vStr.length() > wStr.length()){
                    return 1;
                }
            return 0;
        }
